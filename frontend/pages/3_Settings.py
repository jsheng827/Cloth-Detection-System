...
from violation_settings import (  # type: ignore
    load_violation_settings,
    save_violation_settings,
...
st.markdown("---")
st.markdown('<div class="model-card">', unsafe_allow_html=True)
st.markdown('<div class="model-header">🚫 Violation Type Configuration</div>', unsafe_allow_html=True)

st.write("Select which clothing items should be considered violations. Changes take effect immediately.")

# Load current violation settings
current_violations = load_violation_settings()
...
selected_violations = st.multiselect(
    ...
    options=all_items,
    default=current_violations,
    help="Choose which clothing items should trigger a violation when detected",
)

col1, col2 = st.columns(2)

with col1:
    if st.button("💾 Save Violation Settings", type="primary"):
        if selected_violations:
            save_violation_settings(selected_violations)
            st.success(f"✅ Saved {len(selected_violations)} violation type(s)!")
            st.rerun()
        else:
            st.warning("⚠️ Please select at least one violation type")

with col2:
    if st.button("🔄 Reset to Defaults"):
        reset_to_defaults()
        st.success("✅ Reset to default violation types")
        st.rerun()

# Display current settings
st.markdown("---")
st.subheader("Current Violation Types")
if current_violations:
    st.write(f"**{len(current_violations)} violation type(s) configured:**")
    for i, violation in enumerate(current_violations, 1):
        st.write(f"{i}. {violation}")
else:
    st.info("No violation types configured. All detections will be marked as 'Appropriate'.")
